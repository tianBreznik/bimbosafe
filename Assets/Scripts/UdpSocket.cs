    ...
    private void ReceiveData()
    {
        while (true)
        {
            try
            {
                ...
                //print(">> " + text);
                //telegram.SendTelegramMessage(text, telegram.chat_id);
                //responseHere?.GetPersistentMethodNameInvoke(text, 735504306);
                telegram.modelresponse = text;
                Debug.Log(text);
                //telegram.messagesqueue.Peek().modelresponse = text;
                ProcessInput(text);
            }
            catch (Exception err)
            {
